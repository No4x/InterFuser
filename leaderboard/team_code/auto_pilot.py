...
class AutoPilot(MapAgent):
    ...
    def _get_control(self, target, far_target, near_command, far_command, tick_data):
        ...
        self.brake_tf=self._get_brake_tf()
        ...
        if brake:
            steer *= 0.5
            throttle = 0.0
        pos_tf = self._get_position(tick_data)
        self.gps_buffer.append(pos_tf)
        ...
        self._waypoint_planner_extrapolation.save()
        self.steer_tf=steer
        self.throttle_tf=throttle

        self._save_waypoints() #tf
    ...
    def run_step(self, input_data, timestamp):
        ...
        self.update_gps_buffer(control, data['compass'], data['speed'])
    ...
    def _get_brake_tf(self, vehicle_hazard=None, light_hazard=None, walker_hazard=None, stop_sign_hazard=None):
        actors = self._world.get_actors()
    ...
    def _save_waypoints(self):
        speed = self._get_forward_speed()
        number_of_future_frames = int(self.waypoint_seconds * self.frame_rate_tf)
        vehicle_transform = self._vehicle.get_transform()
        ...
        action = np.array(np.stack([self.steer_tf, self.throttle_tf, self.brake_tf], axis=-1))
        ego_future = []
        ...
        for i in range(number_of_future_frames):
            ...
            # steer_temp = self._get_steer(self.brake, waypoint_route_temp, next_loc_temp, next_yaw_temp, next_speed,
            #                              restore=False)
            ...
            # whole and without
            transform_whole = carla.Transform(
                carla.Location(x=next_loc[0].item(), y=next_loc[1].item(), z=vehicle_transform.location.z))
            bounding_box_whole = carla.BoundingBox(transform_whole.location, extent_org)
            bounding_box_whole.rotation = carla.Rotation(pitch=float(vehicle_transform.rotation.pitch),
                                                         yaw=float(delta_yaws),
                                                         roll=float(vehicle_transform.rotation.roll))
